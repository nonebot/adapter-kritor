...
class FriendInfo(betterproto.Message):
    ...
    ext: Optional["ExtInfo"] = betterproto.message_field(
        99, optional=True, group="_ext"
    )
...
class ProfileCard(betterproto.Message):
    ...
    level: int = betterproto.uint32_field(6)
    birthday: Optional[int] = betterproto.uint64_field(
        7, optional=True, group="_birthday"
    )
    login_day: int = betterproto.uint32_field(8)
    vote_cnt: int = betterproto.uint32_field(9)
    is_school_verified: Optional[bool] = betterproto.bool_field(
        51, optional=True, group="_is_school_verified"
    )
    """以下字段可以不实现"""
...
class SetProfileCardRequest(betterproto.Message):
    nick_name: Optional[str] = betterproto.string_field(
        1, optional=True, group="_nick_name"
    )
    company: Optional[str] = betterproto.string_field(
        2, optional=True, group="_company"
    )
    email: Optional[str] = betterproto.string_field(3, optional=True, group="_email")
    college: Optional[str] = betterproto.string_field(
        4, optional=True, group="_college"
    )
    personal_note: Optional[str] = betterproto.string_field(
        5, optional=True, group="_personal_note"
    )
    birthday: Optional[int] = betterproto.uint32_field(
        6, optional=True, group="_birthday"
    )
...
class GetUidResponse(betterproto.Message):
    uid_map: Dict[int, str] = betterproto.map_field(
...
class GetUinByUidResponse(betterproto.Message):
    uin_map: Dict[str, int] = betterproto.map_field(
...
class FriendServiceBase(ServiceBase):
    ...
    async def set_profile_card(
        self, set_profile_card_request: "SetProfileCardRequest"
    ) -> "SetProfileCardResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def vote_user(
        self, vote_user_request: "VoteUserRequest"
    ) -> "VoteUserResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)

    async def get_uid_by_uin(
        self, get_uid_request: "GetUidRequest"
    ) -> "GetUidResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)

    async def get_uin_by_uid(
        self, get_uin_by_uid_request: "GetUinByUidRequest"
    ) -> "GetUinByUidResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_vote_user(
        self, stream: "grpclib.server.Stream[VoteUserRequest, VoteUserResponse]"
    ) -> None:
        request = await stream.recv_message()
    ...
    async def __rpc_get_uid_by_uin(
        self, stream: "grpclib.server.Stream[GetUidRequest, GetUidResponse]"
    ) -> None:
        request = await stream.recv_message()
